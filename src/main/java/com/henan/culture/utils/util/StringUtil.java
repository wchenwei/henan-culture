...
public class StringUtil {
    ...
    public static Map<Integer, Integer> splitStr2IntMap(String str, String sp1, String sp2) {
        return splitStr2Map(str, sp1, sp2, Integer.class);
    }

    public static <V> Map<Integer, V> splitStr2Map( String str, String sp1, String sp2, Class<V> V) {
        Map<Integer, V> tempMap = Maps.newConcurrentMap();
        if (StringUtils.isBlank(str)) {
            return tempMap;
        }
        for (String info : str.split(sp1)) {
            List<String> strings = splitStr2StrList(info, sp2);
            tempMap.put(Convert.toInt(strings.get(0)), Convert.convert(V, strings.get(1)));
        }
        return tempMap;
    }
    ...
    public static String listStr2Str(List<String> list,String split){
        if (CollUtil.isEmpty(list)) {
            return "";
        }
        StringBuffer sb = new StringBuffer();
    ...
    public static ListMultimap<Integer, Integer> buildListMultimap(String info, String sp1, String sp2, String sp3) {
        ListMultimap<Integer, Integer> typeMap = ArrayListMultimap.create();
        for (String s : info.split(sp1)) {
            String ss[] = s.split(sp2);
            typeMap.putAll(Integer.parseInt(ss[0]), splitStr2IntegerList(ss[1], sp3));
        }
        return typeMap;
    }
}
